...
begin
 ...
 if InputQuery('Создание напоминалки', 'Напомнить:', value) then
  begin
...
then
 begin
  dbgrid1.canvas.brush.color := clsilver; //сегодня
  dbgrid1.Canvas.Font.Color:=clBlack;
 end;
...
// Label2.Caption:=datetostr(date);
 ...
 dbgrid1.Columns[3].Title.caption:='Содержимое';
 //+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
 ...
 while not Queryu.Eof do
  begin
   if strtotime(Queryu.FieldByName('time').AsString)<time() then
    ...
    Form2.Caption:='НАПОМИНАНИЕ';
     TI.BalloonHint:= Queryu.FieldByName('task').AsString;
     TI.BalloonTitle:= 'Система напоминаний:';
     TI.ShowBalloonHint;
     TI.BalloonTimeout:= -1;
  ...
  begin
   if strtotime(Queryu1.FieldByName('time').AsString)<time() then
    ...
    begin
     ...
     Form2.Caption:='ПОВТОРНОЕ НАПОМИНАНИЕ';
     TI.BalloonHint:= Queryu.FieldByName('task').AsString;
     TI.BalloonTitle:= 'Система напоминаний:';
     TI.ShowBalloonHint;
     TI.BalloonTimeout:= -1;
